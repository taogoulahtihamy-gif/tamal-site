...
export default function Simulateur() {
  ...
  const resume = useMemo(() => {
    if (!montantValide) return null

    return [
      {
        titre: "Montant demandé",
        valeur: `${formatMontant(montantNombre)} FCFA`,
      },
      {
        titre: "Durée",
        valeur: `${duree} jours`,
      ...
      {
        titre: "Remboursement normal",
        valeur: `${formatMontant(remboursementNormal)} FCFA`,
      },
      {
        titre: "Après 1 jour de retard",
        valeur: `${formatMontant(remboursementRetard1Jour)} FCFA`,
      },
    ]
  }, [montantValide, montantNombre, duree, remboursementNormal, remboursementRetard1Jour])
      ...
      <div className="mx-auto max-w-6xl px-4">
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm md:p-8">
              ...
              <div className="mt-6 rounded-2xl border border-yellow-300 bg-yellow-50 p-4 text-sm leading-6 text-gray-700">
                ...
                <span className="font-semibold text-yellow-700">
                  {formatMontant(penaliteJournaliere)} FCFA
                </span>{" "}
          ...
          <RevealOnScroll delay={220} direction="left">
            <div className="rounded-3xl border border-yellow-200 bg-white p-6 shadow-sm md:p-8">
              ...
              <p className="mt-3 text-sm leading-6 text-gray-600">
                Ce calcul vous donne une estimation rapide selon les règles
                actuelles du service TAMAL.
              </p>
              ...
              <div className="mt-6 space-y-4">
                {resume ? (
                  resume.map((item) => (
                    <div
                      key={item.titre}
                      className="rounded-2xl border border-gray-200 bg-[#faf9f5] p-4"
                    >
                      <p className="text-sm text-gray-500">{item.titre}</p>
                  ...
                  <div className="rounded-2xl border border-gray-200 bg-[#faf9f5] p-4 text-sm text-gray-500">
                    Saisissez un montant valide pour afficher votre estimation.
                  </div>
                )}
              </div>

              <div className="mt-6 rounded-2xl border border-gray-200 bg-[#faf9f5] p-4">
                <p className="text-sm font-semibold text-gray-800">
                  Conseil TAMAL
                </p>
                <p className="mt-2 text-sm leading-6 text-gray-600">
                  Pour un traitement plus rapide, préparez votre pièce d’identité,
                  votre objet de valeur et une description claire lors de votre
                  demande.
                </p>
              </div>
            </div>
